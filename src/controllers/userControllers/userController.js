...
const getDashboard = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      dashboard: {
        stats: [
          { title: "LIVE PRICE", value: "$0.12", icon: "TrendingUp" },
          { title: "TOTAL DEPOSIT", value: `$${user.totalInvested.toFixed(2)}`, icon: "DollarSign" },
          { title: "WALLET BALANCE", value: `$${user.walletBalance.toFixed(2)}`, icon: "Wallet" },
          { title: "TOTAL EARNINGS", value: `$${user.totalEarnings.toFixed(2)}`, icon: "Coins" },
          { title: "ACTIVE PACKAGE", value: `${user.activePackage} CIP`, icon: "BarChart3" },
          { title: "TEAM", value: `${user.totalReferrals} Users`, icon: "Users" },
        ],
        profitTracker: {
